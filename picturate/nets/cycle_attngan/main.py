...
class CAttnGAN:
    ...
    def load_models(self):
        ...
        if self.pretrained is True:
            ...
            state_dict = torch.load(
                G_NET_path, map_location=lambda storage, loc: storage
            )
        ...
        # Load text encoder
        self.text_encoder = BERT_RNN_ENCODER(
            self.cfg.N_WORDS, cfg=self.cfg, nhidden=self.cfg.TEXT.EMBEDDING_DIM
        )

        if self.pretrained is True:

            if not self.weight_exists("BERT_RNN_ENCODER.pth"):
                pretrained_file_path = "https://drive.google.com/uc?export=1EqcAdwtbM5TU2qyQ7LSsDxlyLzJscKL6"
                save_path = os.path.join(self.cache_directory, "BERT_RNN_ENCODER.pth")
                gdown.download(pretrained_file_path, save_path, quiet=False)

            BERT_RNN_ENCODER_path = os.path.join(
                self.cache_directory, "BERT_RNN_ENCODER.pth"
            )
            state_dict = torch.load(
                BERT_RNN_ENCODER_path, map_location=lambda storage, loc: storage
            )
    ...
    def generate_image(self, sentence, filename):
        ...
        for key in data_dic:
            ...
            fake_imgs, attention_maps, _, _ = self.netG(
